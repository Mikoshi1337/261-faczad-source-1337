...
public:
	...
	void DrawStringd(ImVec2 coord, ImColor color, int flags, ImFont* font, float size_text, const char* message, ...) {
	...
	//	if (a < 0) return;
		...
		if ((a / b) > 3)
			a = b;
		FilledRect(start.x + 1, start.y + 1, end.x * (a / b) - 1, end.y - 1, ImColor(69, 236, 2, 255), 26, ImDrawFlags_RoundCornersAll);
	}
