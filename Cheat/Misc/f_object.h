...
public:
	...
	static f_object get_closest_object(Vector3 from, const char* namee, Vector3 ignore = Vector3::Zero(), Vector3 ignore2 = Vector3::Zero(), Vector3 ignore3 = Vector3::Zero(), bool classname = false, const char* classnamee = (""), float get_dist = 99999.f, bool vis = false, bool y = false) {
	...
	static f_object get_spam1_target(BasePlayer* Player, DWORD64 melee) {
		f_object res = f_object();
		...
		Vector3 lppos = LocalPlayer::Entity()->get_bone_pos(head);
	...
	static f_object get_spam_target(BasePlayer* Player, DWORD64 melee) {
		f_object res = f_object();

		if (LocalPlayer::Entity()->health() <= 0) return res;
		//if (vars::combat::ignore_npc && Player->IsNpc()) return res;
		//if (vars::combat::ignore_sleepers && Player->get_flag(PlayerFlags::Sleeping)) return res;
		//if (vars::combat::ignore_team && LocalPlayer::Entity()->is_teammate(Player->userID())) return res;
		Vector3 prepos = LocalPlayer::Entity()->get_bone_pos(head);
		Vector3 closest_entity = utils::ClosestPoint(LocalPlayer::Entity(), prepos);
		Vector3 closest_local = utils::ClosestPoint(LocalPlayer::Entity(), closest_entity);
		float disttoentity = MaxMeleeDist(melee, false);
		float distfromlocal = MaxMeleeDist(melee, true);
		...
		res.dist = realdist;
		res.entity = (DWORD64)LocalPlayer::Entity();
		res.position = target;
		return res;
	}
