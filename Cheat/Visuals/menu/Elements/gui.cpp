...
bool ui::tab(int num) {
    ...
    window->DrawList->AddText(font(1), font(1)->FontSize, { bb.Min.x + 18, bb.GetCenter().y - text_size(1, icon).y / 2 }, icon_col, icon);
    window->DrawList->AddText({ bb.Max.x - CalcTextSize(label, 0, 1).x - 18,  bb.GetCenter().y - CalcTextSize(label, 0, 1).y / 2 }, text_col, label, FindRenderedTextEnd(label));
...
void ui::handle_alpha_anim() {
    content_anim = ImGui::ImLerp(content_anim, content_anim_dest, anim_speed * 2.5f);
...
void ui::render_login_page() {
    if (tabsl[cur_page].pages.size() == 0 || tabsl[cur_page].pages.size() <= tabsl[cur_page].cur_subtab)
        return;

    tabsl[cur_page].pages[tabsl[cur_page].cur_subtab]();
}
...
void ui::add_login_page(int tab, std::function< void() > code) {
    tabsl[tab] .pages.emplace_back(code);
}
