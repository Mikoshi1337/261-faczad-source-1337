...
//				//	do_dot(ObjectClass, Object, buff, ("assets/prefabs/npc/patrol helicopter/patrolhelicopter.prefab"), D2D1::ColorF::Orange);
//			}
//		}
//	}
//}
...
//						g_Render->DrawString(screen.x, screen.y, ImColor(255, 0, 0, 255), render2::outline | render2::centered_x, pixel_s, 12, ("Corpse"));
...
//					Vector2 screen;
//					if (utils::w2s(pos, screen) && distance <= vars::visuals::base::draw_distance) {
//						g_Render->DrawString(screen.x, screen.y, ImColor(128, 128, 0), render2::outline | render2::centered_x, pixel_s, 12, ("Sleeping Bag"));
//						if (vars::visuals::base::show_distance) {
//							std::string string2;
//							char buf2[256] = { 0 }; sprintf_s(buf2, "%.1fm", distance);
//							string2 = buf2;
...
//					Vector2 screen;
//					if (utils::w2s(pos, screen) && distance <= vars::visuals::base::draw_distance) {
//						g_Render->DrawString(screen.x, screen.y, ImColor(107, 142, 35), render2::outline | render2::centered_x, pixel_s, 12, ("Bed"));
//						if (vars::visuals::base::show_distance) {
//							std::string string2;
//							char buf2[256] = { 0 }; sprintf_s(buf2, "%.1fm", distance);
//							string2 = buf2;
...
//							g_Render->DrawString(screen.x, screen.y, ImColor(255, 0, 0), render2::outline | render2::centered_x, pixel_s, 12, ("Stash [Hidden]"));
//
//						}
//						else {
//
//							g_Render->DrawString(screen.x, screen.y, ImColor(255, 255, 0), render2::outline | render2::centered_x, pixel_s, 12, ("Stash [Open]"));
//
//						}
//						if (vars::visuals::other::show_distance) {
//							std::string string;
...
//
//								g_Render->DrawString(screen.x, screen.y, color, render2::outline | render2::centered_x, pixel_s, 12, ("Chinook Crate"));
//
//							}
//						}
//						else {
//							color = ImColor(32, 178, 170);
//							g_Render->DrawString(screen.x, screen.y, color, render2::outline | render2::centered_x, pixel_s, 12, ("Chinook Crate [Open]"));
//
//						}
//						std::string string;
//						char buf[256] = { 0 }; sprintf_s(buf, "%.1fm", distance);
...
//					g_Render->DrawString(middlePointPlayerFeet.x, middlePointPlayerFeet.y + CurPos + 8.f, color, render2::centered_x | render2::outline, pixel_s, 11, _name.c_str());
//				}
...
//						g_Render->DrawString(middlePointPlayerFeet.x, middlePointPlayerFeet.y + CurPos + 50.f, color, render2::centered_x | render2::outline, pixel_s, 11, wounded.c_str());
//					}
//					else {
...
//					sprintf_s(buf, ("%ls"), ActWeapon->get_name());
//					string += buf;
//					g_Render->DrawString(middlePointPlayerFeet.x, middlePointPlayerFeet.y + CurPos + 18.f, color, render2::centered_x | render2::outline, pixel_s, 11, string.c_str());
//					CurPos += 12.f;
//				}
//
//
...
//					string += buf;
//					g_Render->DrawString(middlePointPlayerFeet.x, middlePointPlayerFeet.y + CurPos + 26.f, color, render2::centered_x | render2::outline, pixel_s, 11, string.c_str());
//					CurPos += 8;
//				}
...
//					if ((int)BP->health() <= 99) {
//						std::string string;
//						char buf[256] = { 0 }; sprintf_s(buf, ("%d"), (int)BP->health());
//						string += buf;
//						g_Render->DrawString(Entity_x + Entity_w - 6.f, Entity_y + (Entity_h * CurOff) - 4, color, render2::centered_x | render2::outline, pixel_s, 11, string.c_str());
//					}
//				}
//			}
//		}
...
//				}
//				if (vars::npc::name) {
//					g_Render->DrawString(middlePointPlayerTop.x, middlePointPlayerTop.y - 15.f, ImColor(vars::colors::npc_name_color), render2::centered_x | render2::outline, pixel_s, 12, ("NPC"));
//				}
//				Item* ActWeapon = BP->GetActiveWeapon();
...
//					std::string string;
//					char buf[256] = { 0 }; sprintf_s(buf, ("%dm"), (int)Math::Distance_3D(LocalPlayer::Entity()->get_bone_pos(head), BP->get_bone_pos(head)));
//					string += buf;
//					g_Render->DrawString(middlePointPlayerFeet.x, middlePointPlayerFeet.y + CurPos2 + 5.f, ImColor(vars::colors::npc_name_color), render2::centered_x | render2::outline, pixel_s, 12, string.c_str());
//					CurPos2 += 8;
//				}
...
namespace otherEsp {
				...
				if (ObjectClass) {
					...
					if (utils::w2s(reinterpret_cast<BaseEntity*>(ObjectClass)->transform()->position()/*utils::GetEntityPosition(gameObject)*/, screen_Pos)) {
						if ((int)Math::Distance_3D(LocalPlayer::Entity()->get_bone_pos(head), reinterpret_cast<BaseEntity*>(ObjectClass)->transform()->position()) <= drawDistance) {
							...
							g_Render->DrawString(screen_Pos.x, screen_Pos.y, color, render2::outline | render2::centered_x, Verdanaboltbyser, 12, string.c_str());
							if (showDistance) {
								std::string string2;
								char buf2[256] = { 0 }; sprintf_s(buf2, "%.1fm", Math::Distance_3D(LocalPlayer::Entity()->get_bone_pos(head), reinterpret_cast<BaseEntity*>(ObjectClass)->transform()->position()));
								string2 = buf2;
								//	DrawList->AddText({ screen_Pos.x - (ImGui::CalcTextSize(string.c_str()).x / 2),screen_Pos.y + 15 }, color, string.c_str());
								g_Render->DrawString(screen_Pos.x, screen_Pos.y + 10, ImColor(128, 128, 128), render2::outline | render2::centered_x, Verdanaboltbyser, 11, string2.c_str());
							}
				...
				if (bradley) {
					...
					if (utils::w2s(pos, screen)) {
						if (health > 0) {
							...
							char buf[256] = { 0 }; sprintf_s(buf, "Bradley APC [%dm]", (int)Math::Distance_3D(LocalPlayer::Entity()->get_bone_pos(head), pos));
							string = buf;
							g_Render->DrawString(screen.x, screen.y, ImColor(255, 0, 0, 255), render2::outline | render2::centered_x, Verdanaboltbyser, 11, string.c_str());
		...
		if (vars::visuals::other::corpse) {
			if (strstr(buff, ("player_corpse.prefab"))) {
				...
				if (corpse) {
					...
					Vector2 screen;
					if (utils::w2s(pos, screen) && distance <= vars::visuals::other::draw_distance) {
						g_Render->DrawString(screen.x, screen.y, ImColor(255, 0, 0, 255), render2::outline | render2::centered_x, Verdanaboltbyser, 11, ("Corpse"));
						if (vars::visuals::other::show_distance) {
							std::string string2;
							char buf2[256] = { 0 }; sprintf_s(buf2, "%.1fm", distance);
							string2 = buf2;
							g_Render->DrawString(screen.x, screen.y + 10, ImColor(128, 128, 128), render2::outline | render2::centered_x, Verdanaboltbyser, 11, string2.c_str());
			...
			if (strstr(buff, ("cupboard.tool.deployed.prefab"))) {
				...
				if (cupboard) {
					...
					if (utils::w2s(pos, screen) && distance <= vars::visuals::base::draw_distance) {
						...
						char buf[256] = { 0 }; sprintf_s(buf, "Tool Cupboard | %d hours", upkeep / 60);
						string = buf;
						g_Render->DrawString(screen.x, screen.y, ImColor(0, 255, 0, 255), render2::outline | render2::centered_x, Verdanaboltbyser, 11, string.c_str());
		...
		if (vars::visuals::base::sleeping_bag) {
			if (strstr(buff, ("sleepingbag_leather_deployed.prefab"))) {
				...
				if (bag) {
					...
					Vector2 screen;
					if (utils::w2s(pos, screen) && distance <= vars::visuals::base::draw_distance) {
						g_Render->DrawString(screen.x, screen.y, ImColor(128, 128, 0), render2::outline | render2::centered_x, Verdanaboltbyser, 11, ("Sleeping Bag"));
						if (vars::visuals::base::show_distance) {
							std::string string2;
							char buf2[256] = { 0 }; sprintf_s(buf2, "%.1fm", distance);
							string2 = buf2;
		...
		if (vars::visuals::base::bed) {
			if (strstr(buff, ("bed_deployed.prefab"))) {
				...
				if (bag) {
					...
					Vector2 screen;
					if (utils::w2s(pos, screen) && distance <= vars::visuals::base::draw_distance) {
						g_Render->DrawString(screen.x, screen.y, ImColor(107, 142, 35), render2::outline | render2::centered_x, Verdanaboltbyser, 11, ("Bed"));
						if (vars::visuals::base::show_distance) {
							std::string string2;
							char buf2[256] = { 0 }; sprintf_s(buf2, "%.1fm", distance);
							string2 = buf2;
			...
			if (strstr(buff, ("small_stash_deployed.prefab"))) {
				...
				if (stash) {
					...
					if (utils::w2s(pos, screen) && distance <= vars::visuals::other::draw_distance) {
						...
						if (((IsHidden)(vars::stor::gBase + CO::IsHidden))(stash)) {
							g_Render->DrawString(screen.x, screen.y, ImColor(255, 0, 0), render2::outline | render2::centered_x, Verdanaboltbyser, 11, ("Stash [Hidden]"));

						}
						else {

							g_Render->DrawString(screen.x, screen.y, ImColor(255, 255, 0), render2::outline | render2::centered_x, Verdanaboltbyser, 11, ("Stash [Open]"));
						...
						if (vars::visuals::other::show_distance) {
							...
							char buf[256] = { 0 }; sprintf_s(buf, "%.1fm", distance);
							string = buf;
							g_Render->DrawString(screen.x, screen.y + 10, ImColor(128, 128, 128), render2::outline | render2::centered_x, Verdanaboltbyser, 11, string.c_str());
						}
					}
				}
			}
					...
					if (utils::w2s(pos, screen)) {
						...
						if (!((IsFullyHacked)(vars::stor::gBase + CO::IsFullyHacked))(crate)) {
							if (((IsBeingHacked)(vars::stor::gBase + CO::IsBeingHacked))(crate)) {
								...
								char buf[256] = { 0 }; sprintf_s(buf, "Chinook Crate | %ls", _name);
								string = buf;
								g_Render->DrawString(screen.x, screen.y, color, render2::outline | render2::centered_x, Verdanaboltbyser, 11, string.c_str());

							}
							else {
								color = ImColor(255, 69, 0);

								g_Render->DrawString(screen.x, screen.y, color, render2::outline | render2::centered_x, Verdanaboltbyser, 11, ("Chinook Crate"));

							}
						}
						else {
							color = ImColor(32, 178, 170);
							g_Render->DrawString(screen.x, screen.y, color, render2::outline | render2::centered_x, Verdanaboltbyser, 11, ("Chinook Crate [Open]"));
	...
	////						char buf[256] = { 0 }; sprintf_s(buf, "%ls", english->str);
	////						string = buf;
	////						g_Render->DrawString(screen.x, screen.y, ImColor(255, 255, 255, 255), render2::outline | render2::centered_x, Verdanaboltbyser, 12, string.c_str());
	////						if (vars::visuals::other::show_distance) {
	////							std::string string2;
	////							char buf[256] = { 0 }; sprintf_s(buf, "%.1fm", distance);
	////							string2 = buf;

	////							g_Render->DrawString(screen.x, screen.y + 10, ImColor(128, 128, 128), render2::outline | render2::centered_x, Verdanaboltbyser, 12, string2.c_str());
	////						}
	////					}
	////				}
	////			}
...
void OOF(BasePlayer* ply, ImColor _color) {
	...
	if (vars::players::oof_arrows) {
		if (utils::OOF(ply)) {
			...
			sprintf_s(buf1, ("[ %dm ]"), (int)Math::Distance_3D(LocalPlayer::Entity()->get_bone_pos(head), ply->get_bone_pos(head)));
			string1 += buf1;
			Vector2 tp0 = CosTanSinLineH(angleYawRad, 5.f, vars::stuff::ScreenWidth / 2, vars::stuff::ScreenHeight / 2, 270.f);
			Vector2 tp1 = CosTanSinLineH(angleYawRad + 2.f, 5.f, vars::stuff::ScreenWidth / 2, vars::stuff::ScreenHeight / 2, 260.f);
			Vector2 tp2 = CosTanSinLineH(angleYawRad - 2.f, 5.f, vars::stuff::ScreenWidth / 2, vars::stuff::ScreenHeight / 2, 260.f);
			...
			if (vars::players::Outstyle == 3) {
				...
				g_Render->DrawString(tp1.x + 5, tp1.y - 20.f, _color, render2::centered_x | render2::outline, Verdanaboltbyser, 11, _name.c_str());
				g_Render->DrawString(tp1.x + 7, tp1.y + 30.f, _color, render2::centered_x | render2::outline, Verdanaboltbyser, 11, string1.c_str());
				if (ActWeapon)
				{
					g_Render->DrawString(tp1.x + 7, tp1.y + 20.f, _color, render2::centered_x | render2::outline, Verdanaboltbyser, 11, string.c_str());
				}
				else
				{
					g_Render->DrawString(tp1.x + 7, tp1.y + 20.f, _color, render2::centered_x | render2::outline, Verdanaboltbyser, 11, "Empty");
				}
			}
...
void ESP(BasePlayer* BP, const ImColor color) {
	...
	if (utils::w2s(BP->get_bone_pos(r_foot), tempFeetR) && utils::w2s(BP->get_bone_pos(l_foot), tempFeetL)) {
		...
		if (utils::w2s(BP->get_bone_pos(head) + Vector3(0, 0.2f, 0), tempHead)) {
			...
			float Entity_y = tempFeet.y;
			float Entity_w = Entity_h / 2;
			Vector3 middlePointWorld = BP->get_bone_pos(l_foot).midPoint(BP->get_bone_pos(r_foot));//cachedBones[BP->userID()]->l_foot->position.midPoint(cachedBones[BP->userID()]->r_foot->position
			Vector3 middlePointWorld_ = BP->get_bone_pos(head).midPoint(BP->get_bone_pos(head));//cachedBones[BP->userID()]->head->position.midPoint(cachedBones[BP->userID()]->head->position
			int CurPos = 0;
			int CurPos2 = 0;
			Vector2 middlePointPlayerTop;
			Vector2 middlePointPlayerFeet;
			if (utils::w2s(middlePointWorld_, middlePointPlayerTop) && utils::w2s(middlePointWorld, middlePointPlayerFeet)) {
				...
				if (vars::players::name) {
					...
					{
						std::string _name(CStringA(BP->_displayName()));
						g_Render->DrawString(middlePointPlayerFeet.x, middlePointPlayerFeet.y + CurPos + 8.f, color, render2::centered_x | render2::outline, Verdanaboltbyser, 11, _name.c_str());
					}

					if (vars::players::namePos == 1)
					{
						std::string _name(CStringA(BP->_displayName()));
						g_Render->DrawString(middlePointPlayerFeet.x, middlePointPlayerTop.y - 15.f, color, render2::centered_x | render2::outline, Verdanaboltbyser, 11, _name.c_str());
					}
				...
				if (vars::players::weapon && ActWeapon) {
					...
					{
						g_Render->DrawString(middlePointPlayerFeet.x, middlePointPlayerFeet.y + CurPos + 8.f, color, render2::centered_x | render2::outline, Verdanaboltbyser, 11, string.c_str());
					}
					if (vars::players::weaponPos == 0 && vars::players::weaponPos == vars::players::namePos)
					{
						g_Render->DrawString(middlePointPlayerFeet.x, middlePointPlayerFeet.y + CurPos + 18.f, color, render2::centered_x | render2::outline, Verdanaboltbyser, 11, string.c_str());
					}
					if (vars::players::weaponPos == 1 && vars::players::weaponPos != vars::players::namePos)
					{
						g_Render->DrawString(middlePointPlayerFeet.x, middlePointPlayerTop.y - 15.f, color, render2::centered_x | render2::outline, Verdanaboltbyser, 11, string.c_str());
					}
					if (vars::players::weaponPos == 1 && vars::players::weaponPos == vars::players::namePos)
					{
						g_Render->DrawString(middlePointPlayerFeet.x, middlePointPlayerTop.y - 25.f, color, render2::centered_x | render2::outline, Verdanaboltbyser, 11, string.c_str());
					}
				...
				else if (vars::players::weapon)
				{
					...
					if (vars::players::weaponPos == 0 && vars::players::weaponPos != vars::players::namePos)
					{
						g_Render->DrawString(middlePointPlayerFeet.x, middlePointPlayerFeet.y + CurPos + 8.f, color, render2::centered_x | render2::outline, Verdanaboltbyser, 11, "Empty");
					}
					if (vars::players::weaponPos == 0 && vars::players::weaponPos == vars::players::namePos)
					{
						g_Render->DrawString(middlePointPlayerFeet.x, middlePointPlayerFeet.y + CurPos + 18.f, color, render2::centered_x | render2::outline, Verdanaboltbyser, 11, "Empty");
					}
					if (vars::players::weaponPos == 1 && vars::players::weaponPos != vars::players::namePos)
					{
						g_Render->DrawString(middlePointPlayerFeet.x, middlePointPlayerTop.y - 15.f, color, render2::centered_x | render2::outline, Verdanaboltbyser, 11, "Empty");
					}
					if (vars::players::weaponPos == 1 && vars::players::weaponPos == vars::players::namePos)
					{
						g_Render->DrawString(middlePointPlayerFeet.x, middlePointPlayerTop.y - 25.f, color, render2::centered_x | render2::outline, Verdanaboltbyser, 11, "Empty");
					}
				...
				if (vars::players::distance) {
					...
					if (vars::players::weaponPos == 0 && vars::players::weaponPos == vars::players::namePos)
					{
						g_Render->DrawString(middlePointPlayerFeet.x, middlePointPlayerFeet.y + CurPos + 26.f, color, render2::centered_x | render2::outline, Verdanaboltbyser, 11, string.c_str());
					}
					if (vars::players::weaponPos == 1 && vars::players::weaponPos != vars::players::namePos)
					{
						g_Render->DrawString(middlePointPlayerFeet.x, middlePointPlayerTop.y - 25.f, color, render2::centered_x | render2::outline, Verdanaboltbyser, 11, string.c_str());
					...
					if (vars::players::weaponPos == 1 && vars::players::weaponPos == vars::players::namePos)
					{
						g_Render->DrawString(middlePointPlayerFeet.x, middlePointPlayerTop.y - 35.f, color, render2::centered_x | render2::outline, Verdanaboltbyser, 11, string.c_str());
					}
				...
				if (vars::players::healthbar) {
					...
					if ((int)BP->health() <= 99) {
						...
						char buf[256] = { 0 }; sprintf_s(buf, ("%d"), (int)BP->health());
						string += buf;
						g_Render->DrawString(Entity_x + Entity_w - 6.f, Entity_y + (Entity_h * CurOff) - 4, color, render2::centered_x | render2::outline, Verdanaboltbyser, 11, string.c_str());
					}
				}
		...
		if (utils::w2s(BP->get_bone_pos(jaw) + Vector3(0.f, 0.16f, 0.f), tempHead)) {
...

			{
				...
				if (utils::w2s(middlePointWorld_, middlePointPlayerTop) && utils::w2s(middlePointWorld, middlePointPlayerFeet)) {
					...
					if (vars::npc::name) {
						g_Render->DrawString(middlePointPlayerTop.x, middlePointPlayerTop.y - 15.f, ImColor(vars::colors::npc_name_color), render2::centered_x | render2::outline, Verdanaboltbyser, 11, ("NPC"));
					}
					Item* ActWeapon = BP->GetActiveWeapon();
					if (vars::npc::weapon && ActWeapon) {
						std::string string; char buf[256] = { 0 };
						sprintf_s(buf, ("%ls"), ActWeapon->get_name());
						string += buf;
						g_Render->DrawString(middlePointPlayerFeet.x, middlePointPlayerFeet.y + CurPos2 + 5.f, ImColor(vars::colors::npc_name_color), render2::centered_x | render2::outline, Verdanaboltbyser, 11, string.c_str());
						CurPos2 += 10;
					}
