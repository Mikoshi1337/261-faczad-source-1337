...
namespace notify {
    void render()
    {
        if (notifications.empty())
            return;
        float notification_spacing = 10.0f; // Расстояние между уведомлениями
        float last_y = 0;
        ...
        for (size_t i = 0; i < notifications.size(); i++)
        {
            ...
            float box_w = (float)fabs(0 - (all_textSize.x + 20));
            float box_width = notify.x + all_textSize.x + 90;
