...
public:
	...
	static void LogExplosion(std::string type, Vector3 pos) {
		...
		if (!explosionCollision) {
			...
			explosion.name = StringFormat::format(("%s Raid"), type.c_str());
	...
	static void RenderExplosions() {
		for (int i = 0; i < LogSystem::loggedExplosions.size(); i++) {
			...
			//	OxyRender::Text(explPos,D2D1::ColorF::Red,			true,true,StringConverter::ToUnicode(StringFormat::format(("%s [%1.0fm] [%d]"),explosion.name.c_str(),Math::Distance_3D(explosion.position, LocalPlayer::Entity()->get_bone_pos(head)),(int)(timee - (get_time_since_startup() - LogSystem::loggedExplosions[i].timeSince)))).c_str()

				//);
			
				std::string string;
				char buf[256] = { 0 }; sprintf_s(buf, ("%s [%1.0fm] [%d]"), explosion.name.c_str(), Math::Distance_3D(explosion.position, LocalPlayer::Entity()->get_bone_pos(head)), (int)(timee - (get_time_since_startup() - LogSystem::loggedExplosions[i].timeSince)));
				string += buf;
